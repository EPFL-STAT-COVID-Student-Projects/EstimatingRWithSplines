...
simulation <- function(
  ...
  true_mean_GI = (t_E+t_I)
  true_var_GI = 2*(true_mean_GI/2)^2
  
  for(intervention_time_1 in intervention_time_1){
    for(decrease_duration in days_intervention_to_min){
      for(intervention_time_2 in intervention_time_2){
        for(increase_duration in days_to_Rt_rise){
          ## Specify time-varying R0
          arnaught <- specify_arnaught(R0_vec = c(pre_intervention_R0, intervention_R0, partially_lifeted_R0), 
            ...
            for(model_type in model_types) {
              sim_df <- if(model_type == 'sir') {
                ...
                simulate_seir_example(
                  arnaught = arnaught,
                  t_E = t_E, t_I = t_I,
                  N = N, E_init = E_init, I_init = I_init,
                  n_t = n_t,
                  method = method
                ) %>%
                  mutate(
